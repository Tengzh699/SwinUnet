...
def compute_val_loss(loader, model, loss_fn):
    model.eval()
    total_loss = 0
    with torch.no_grad():
        for x, y in loader:
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        val_loss = compute_val_loss(val_loader, model, loss_fn)
    ...
    smooth_dice = smooth_curve(val_dices)
    ...
    # ===== Loss 曲线 =====
    plt.subplot(1, 2, 1)
    plt.plot(epochs_range, train_losses, label="Train Loss", color='red', alpha=0.5)
    plt.plot(epochs_range, smooth_train, label="Smooth Train Loss", linestyle='--')

    if len(val_losses) > 0:
        plt.plot(epochs_range, val_losses, label="Val Loss", color='orange', alpha=0.5)
        plt.plot(epochs_range, smooth_val, label="Smooth Val Loss", linestyle='--')
    ...
    # ===== Dice 曲线 =====
    plt.subplot(1, 2, 2)
    plt.plot(epochs_range, val_dices, label="Dice", color='blue', alpha=0.5)
    plt.plot(epochs_range, smooth_dice, label="Smooth Dice", linestyle='--')
    ...
    # 保存图像
    plot_path = os.path.join(exp_dir, "training_curve2.png")
    plt.savefig(plot_path, dpi=300)
    plt.show()
